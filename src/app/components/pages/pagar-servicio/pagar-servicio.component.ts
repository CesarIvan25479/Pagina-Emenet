...
export class PagarServicioComponent {
  ...
  async buscar(): Promise<void> {
    ...
    try {
      ...
      this.mesesPago = this.pagarService.generarMesesPendientes(
        estadoCuenta[estadoCuenta.length - 1].mensualidad,
        costoMes
      );
  ...
  async buscarPorNombre(): Promise<void> {
    ...
    try {
      ...
      const res: any = await firstValueFrom(this.apiClients.infoCliente(this.busquedaNombre.trim()));

      if (Array.isArray(res)) {
        this.clientesEncontrados = res;
      } else if (res?.cliente) {
        this.clientesEncontrados = [res.cliente];
      } else {
        this.clientesEncontrados = [];
      }
